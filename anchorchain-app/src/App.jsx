...
function cloudSave() {
  ...
  // Short debounce to batch rapid taps, then save immediately.
  saveTimer = setTimeout(() => { window.__acLastLocalEdit = Date.now(); cloudSaveNow(); }, 300);
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      if (stopped) return;
      // Skip if the user edited very recently (let their save fully complete first)
      const sinceEdit = Date.now() - (window.__acLastLocalEdit || 0);
      if (sinceEdit < 5000) return;
    ...
    // Also refresh when the tab regains focus — but NOT if the user just edited
    // (otherwise a quick tap-then-return would wipe an unsaved change).
    const onFocus = () => {
      const sinceEdit = Date.now() - (window.__acLastLocalEdit || 0);
      if (sinceEdit < 3000) return; // protect very recent edits
      tick();
    };
