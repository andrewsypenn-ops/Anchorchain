...
// Paste your Anthropic API key here (starts with sk-ant-) to enable photo-to-number reading
...
function CEODashboard({ yesterdayPct, streak, date }) {
  ...
  // Read a number from an uploaded photo using Claude vision
  const readPhotoNumber = (cardKey, file) => {
    ...
    reader.onload = async () => {
      try {
          ...
          body: JSON.stringify({
            ...
            messages: [{
              ...
              content: [
                ...
                { type: "text", text: "Look at this image of a dental practice schedule or production report. Find the total PRODUCTION dollar amount (the main money/revenue figure). Reply with ONLY the number, no dollar sign, no words, no commas. For example: 12450. If you can't find a production number, reply with: NONE" }
              ]
            }]
          })
        });
        ...
        if (num && text !== "NONE") {
          setMetrics(prev => {
            const m = { ...prev, [cardKey]: { ...prev[cardKey], value: num } };
            saveDashboard(m);
            return m;
          });
        }
